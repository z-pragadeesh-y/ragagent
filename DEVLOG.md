...
## Plan 3: Production Deployment & Live Enhancements ✅ Complete

Plan 3 picks up after Plan 2's closeout and covers everything that turned RAGAgent from "a working system verified via `python -m graph.test_cases`" into a live, publicly deployed product: a real frontend, a cloud deployment migration forced by a genuine production failure, and four follow-up priorities (dynamic document ingestion, a real-time Pipeline Inspector, a shared-retrieval confirmation pass, and a web-search fallback) each found and fixed through the same process as every step before it — build, test with real cases, find real bugs, fix them, document honestly.

### Frontend Build ✅ Complete

**What was done:** Built a standalone React (Vite) frontend, `ragagent-frontend`, as its own separate GitHub repository — a single-component app (`App.jsx`) with a dark, neon purple/cyan theme (Framer Motion animations), an animated "AI Core Orb," a Pipeline Inspector panel showing pipeline progress, animated citation rendering, and a document-upload control. Deployed to Vercel with auto-deploy on push, live at a public `.vercel.app` URL, talking to the backend via a hardcoded `API_BASE` constant.

**Bugs found and fixed during integration:** (1) CORS — the FastAPI backend had no CORS policy at all, so the deployed frontend's requests were silently rejected by the browser; fixed by adding `CORSMiddleware` to `api/main.py`, explicitly allowing both `localhost:5173` (local dev) and the real Vercel URL. (2) A Pydantic v2 behavior change — `ChatRequest.thread_id: str = None` doesn't actually accept an explicit JSON `null` in Pydantic v2 the way it silently did in v1; fixed by declaring it `Optional[str] = None`. (3) The frontend was hardened to fail gracefully (a clear "couldn't reach the backend" message) rather than crash on any malformed or unexpected API response.

**Result:** RAGAgent has a real, live, publicly-accessible UI for the first time, not just a `curl`-driven API.

### Migration: Railway → Google Cloud Run ✅ Complete

**What was done:** The backend was originally deployed on Railway. In production, it repeatedly failed with "Couldn't reach the backend" errors from the live frontend. Root-caused to Railway's 1GB RAM cap being exceeded — the embedding model, cross-encoder reranker, BM25 index, and multi-provider LLM manager together don't fit comfortably in 1GB, causing an OOM kill-and-restart cycle under real traffic. Rather than keep patching around a hard memory ceiling, the entire backend was migrated to Google Cloud Run, which offers a larger free-tier memory allowance and clean scale-to-zero behavior (an idle container simply stops, rather than getting OOM-killed under load).

**Deployment bugs found and fixed, each a genuine Cloud-Run-specific gotcha:**
1. The Dockerfile's `CMD` used JSON-array form with a hardcoded `--port 8000`. Cloud Run injects its own listening port via the `$PORT` environment variable (8080), and JSON-array `CMD` can't expand shell environment variables at all — fixed by switching to shell-form `CMD uvicorn api.main:app --host 0.0.0.0 --port ${PORT:-8000}`.
2. The `.chroma` vector database directory was silently excluded from every Cloud Build upload. `gcloud` falls back to `.gitignore`'s exclusion rules when no `.gcloudignore` file exists, and `.gitignore` (correctly) excludes `.chroma` from *git* — but Cloud Build needs it *included* in the deployable image. Fixed by adding an explicit `.gcloudignore` that overrides this and keeps `.chroma`.
3. `GROQ_API_KEY`/`NVIDIA_API_KEY` were never set as Cloud Run environment variables at all — they'd only ever existed in the now-deleted Railway config. Fixed via `gcloud run services update --set-env-vars`.
4. The cross-encoder reranker was downloading its weights from Hugging Face at runtime on every cold start, and under Cloud Run's shared outbound IP range this reliably hit Hugging Face's anonymous-request rate limit (429). Fixed by baking both the reranker and the embedding model into the Docker image at *build* time (two added `RUN` lines that pre-download the weights), so no network call to Hugging Face happens at runtime at all.

**Cost-safety measures put in place** (a real, ongoing operational requirement, not a one-time fix): a ₹1 Cloud Billing budget alert (50/90/100% email thresholds) was set up and confirmed working. A full auto-disable-billing safety net (Pub/Sub + Cloud Function that would automatically disable billing if the budget were ever exceeded) was attempted but abandoned after a persistent `pubsub.topics.create` permission error that persisted even after granting the correct IAM role — judged disproportionate effort for a personal/learning project, and the combination of the working budget-alert email plus Cloud Run's `--max-instances=1` cap (limiting worst-case blast radius even under a traffic spike or abuse) was accepted as sufficient protection instead.

**Result:** The live backend now runs at a stable Cloud Run URL (`us-central1`, 2Gi memory, 2 CPU, `--max-instances=1`, 900s timeout), with the frontend's `API_BASE` updated and redeployed to match. Railway's project was fully deleted, including its GitHub webhook, to avoid any lingering confusion about which platform is authoritative.

**A genuine security incident, handled correctly:** midway through this work, real `GROQ_API_KEY`/`NVIDIA_API_KEY` values were accidentally pasted in plaintext into a chat session. Both keys were immediately rotated (revoked and regenerated on their respective provider consoles) and Cloud Run was updated with the new values — treated as a real incident requiring real remediation, not something to just note and move past.

### Priority 1: Dynamic Document Ingestion ✅ Complete (one explicitly deferred limitation)

**What was done:** Users can now upload their own document (PDF/TXT/MD) through the live app, and ask questions against it — either alone, or combined with the permanent 5-domain knowledge base in the same conversation. The upload pipeline mirrors the permanent corpus's own processing as closely as possible rather than inventing a parallel path: extract + clean (`ingestion/pdf_preprocessor.py`) → structure into the same section-marker markdown format the 5 permanent documents use (`ingestion/markdown_generator.py`, windowed LLM calls so large documents don't risk silent truncation) → classify domain and persist to disk (`ingestion/metadata_enricher.py`, correctly falling back to `domain_tag="uploaded"` rather than forcing a weak match onto one of the 5 fixed domains) → the exact same `structured_chunker.chunk_documents_structured()` function already used for the permanent corpus → a per-thread, isolated Chroma collection and BM25 index (`ingestion/session_store.py`).

**Real bugs found via live re-testing after the initial build, each fixed and re-verified:**
1. The router misclassified a genuinely in-KB niche economics question (about rare-earth "magnet-4" elements) as `out_of_scope`, and at the time, `route_after_classification` only allowed retrieval to proceed on an `out_of_scope` verdict if the thread happened to have an uploaded document — otherwise it hard-stopped, permanently blocking a real answer with no recourse. Fixed by making retrieval always run regardless of router category, trusting CRAG's evidence-based grading as the real gate instead of a single-shot topic classification.
2. `generate_node` hedged ("not explicitly stated") on multi-part comparison questions (e.g. comparing the AI RMF's Govern/Map/Measure functions) even when the retrieved chunks did contain the answer, just split across passages. Fixed by raising `RETRIEVAL_K` from 4 to 6 and tightening the generation prompt to state the answer directly when the context supports it.
3. A question deliberately combining an uploaded document with a permanent-KB domain (e.g. "how does the UDHR's approach to human dignity compare to WHO life expectancy stats") failed outright, since the router only knew about the fixed 5 topics and had no concept of an uploaded document as a valid `decompose` target. Fixed by having `graph/router.py`'s `route_query()` accept a `has_uploaded_doc` flag and explicitly telling the router prompt to treat an attached upload as a valid second source.
4. Uploading a larger document (a 935KB PDF) hit a hard `504 upstream request timeout`, even after raising Cloud Run's own request timeout to 900 seconds. The `/upload` endpoint was made asynchronous — it now returns immediately with `{"status": "processing"}` and runs the actual ingestion in a background thread, with a new `GET /upload/status/{thread_id}` endpoint for the frontend to poll — but the large upload *still* fails, now timing out client-side after 5 minutes of polling with the background job apparently never completing or reporting status. The root cause (candidates: Cloud Run silently recycling the container mid-background-thread since nothing keeps the request "open" from its perspective, or `markdown_generator.py`'s genuinely-sequential windowed LLM calls simply being too slow at this document size) was **not** further diagnosed — explicitly deferred as a known limitation requiring a real job-queue architecture change, not a prompt-level fix, rather than continuing to debug an increasingly deep async/infrastructure problem for a secondary feature.

**Verification:** every fix above was verified via a fresh `curl` request against the live Cloud Run backend after deploying, confirming a correct, well-cited answer where there previously wasn't one — not just re-reading the code and assuming it was fixed.

**Files changed:** `graph/nodes.py`, `graph/router.py`, `api/main.py`, `ragagent-frontend/src/App.jsx` (polling logic for the async upload status).

**Result:** small-to-normal document uploads, upload-plus-permanent-KB combined questions, and the permanent 5-domain KB's own retrieval quality are all fixed, deployed, and live-verified. Large-document upload (roughly 900KB+) remains a known, explicitly deferred gap.

### Priority 2: Real Per-Node Pipeline Inspector ✅ Complete

**What was done:** The frontend's "Pipeline Inspector" panel previously animated through a fixed 6-stage progress bar on a hardcoded `setTimeout` schedule — cosmetic, and actively misleading on any branch the timer didn't know about (a `direct_answer` turn skips retrieval entirely; an `out_of_scope` turn stops early; a failed grading pass loops `retrieve` a second time). This was replaced with real, live streaming of the graph's actual execution: `/chat/stream` now drives `graph.stream(state, config, stream_mode="updates")` — LangGraph's *synchronous* streaming call, which yields one `{node_name: state_update}` dict per node as it genuinely completes, with no async-checkpointer migration needed — inside a background thread, bridging each event onto a thread-safe `queue.Queue` that the async SSE generator drains via `loop.run_in_executor`. The frontend switched from a single-shot `/chat` call to reading this SSE stream manually via `fetch()` + `getReader()` (`EventSource` can't send a POST body, so it wasn't an option), rendering exactly the nodes that actually ran, in the order they actually ran, per question.

**A real bug found and fixed during testing:** the rewritten `PipelineInspector` component still referenced an old, now-removed `result` prop in a trailing summary-footer block, crashing the entire app to a blank white screen (`ReferenceError: result is not defined`) the first time a real question was asked against it. Fixed by adding a proper `summary` prop, wired from the final SSE event's data.

**Verification:** tested first locally (`uvicorn` + Vite dev server), confirming the full real node sequence streamed and rendered correctly with citations attached, then re-verified end-to-end against the *live* Vercel frontend talking to the *live* Cloud Run backend — not just locally.

**Files changed:** `api/main.py` (new `_stream_graph_worker`, `NODE_STAGE_META`, `_serialize_node_update`), `ragagent-frontend/src/App.jsx` (`PipelineInspector` rewritten, `sendMessage` switched to SSE).

**Result:** the Pipeline Inspector now shows what the system is actually doing, live, rather than a fixed animation loosely correlated with it.

### Priority 3: Shared Retrieval Architecture ✅ Confirmed (no code changes needed)

**What was done:** this priority turned out to already be correctly implemented as a direct byproduct of Priority 1's design (uploaded-document chunks were always meant to be fused into the *same* RRF-fusion-and-rerank ranking call as the permanent corpus, never retrieved separately and stitched together afterward) — so this step was a live confirmation pass, not new development.

**Verification:** uploaded a small test document via the live app, then asked a question specific enough that only the uploaded document could plausibly answer it. The `retrieve` node's real-time event showed the uploaded file's name listed *alongside* two permanent-KB document names in the exact same retrieval call (`doc_count: 6`), proving true single-call fusion rather than two separate retrievals merged after the fact — and CRAG correctly narrowed those 6 candidates down to the 1 actually-relevant chunk (the uploaded document), which the final answer correctly cited.

**Result:** confirmed working exactly as designed, with no code changes required.

### Latency Investigation ✅ Resolved (local development issue, not a live-site bug)

**What was done:** local development sessions were experiencing response times that compounded turn-over-turn within a single session (27 seconds, then 45, then 1.4 minutes, then over 2 minutes) — investigated rather than dismissed as "just slow LLM providers."

**Root cause found:** `uvicorn --reload`'s default file watcher (WatchFiles) watches the *entire* project directory by default, including `checkpoints.sqlite` (the LangGraph checkpointer's database) and the `.chroma/` vector store — both of which are written to on literally every single chat request. Each write was being misread as a source-code change, silently restarting the entire server process mid-session, which re-triggered the LLM manager's initialization and reloaded the embedding/reranking models from disk from scratch — explaining the exact compounding pattern observed.

**Fix:** scope the reload watcher to code directories only: `uvicorn api.main:app --reload --reload-dir api --reload-dir graph --reload-dir llm --reload-dir ingestion --reload-dir feedback`. Confirmed via a clean single-startup session afterward, with no further mid-session restarts.

**A related, smaller finding investigated and correctly ruled out as a real bug:** `reformulate_query_node` calls the LLM manager at `temperature=0.3`, versus `temperature=0` used everywhere else in the same lane. Since the LLM manager cache key is `(lane_name, temperature)`, this is technically a cache miss — but only the *first* time a reformulate loop ever runs in a given process, after which it's cached too. A one-time, negligible cost, not a per-call rebuild, and correctly left unchanged.

**Result:** live Cloud Run latency (the genuine 1-2 minute cold-start delays seen when the service scales from zero) is a separate, expected cost of running with `--max-instances=1` and scale-to-zero — not a code bug, and left as an accepted trade-off against the alternative (`--min-instances=1`, an always-warm container that costs continuously), which would conflict with the project's explicit zero-tolerance-cost stance from the Cloud Run migration.

### Priority 4: Web Search Fallback via Tavily ✅ Complete

**What was done:** questions the knowledge base genuinely can't answer (`out_of_scope`, confirmed by CRAG's grading — not just the router's initial guess) previously received a fixed, canned refusal message. This was replaced with a real web search: a new `graph/web_search_node.py` calls the Tavily API and wraps each result as a LangChain `Document` with `domain_tag="web"` (source = the real URL, title = Tavily's real page title) — deliberately reusing the *exact same* metadata shape the existing `citation_node.py` already expects for knowledge-base chunks, so citation validation and References-list building needed zero special-casing at all to support web-sourced answers. Every web-sourced answer is generated with an explicit, hardcoded opening ("Based on a web search: ...") so it's never confused with a knowledge-base-grounded answer.

**A real design decision, made explicitly rather than defaulted into:** `scope_guard_node.py`'s output guardrail previously flagged *anything* outside the 5 KB domains as out-of-scope — which would have auto-discarded every single web-search answer on sight, defeating the entire point of this priority. Rather than simply bypassing scope_guard for web answers (which would leave web answers with zero opinion/advice-drift protection), its prompt was deliberately *widened*: it now only flags genuine ungrounded opinion or advice, and explicitly treats an answer clearly labeled as web-sourced as in-scope by construction.

**Three real bugs found via live testing, each fixed and re-verified before this priority was considered done:**
1. `grade_documents_node`'s single-call LLM grading occasionally false-positived "yes" on completely irrelevant knowledge-base chunks (e.g. climate-report chunks scored as relevant to a weather question). Since CRAG technically "passed," the question never reached the web-search node at all — `generate_node` produced its own honest, instructed refusal ("I don't have enough information to answer that"), and that dead-end answer was simply returned to the user. Fixed with a safety-net conditional edge: a new `route_after_generate()` checks for that exact refusal phrase and redirects to the web-search node instead of finishing the turn — verified by re-running the same weather question and confirming it now correctly falls through to a real web search.
2. The web-search query was initially built from `rewritten_question`, but by the time the `out_of_scope` node is reached, that question has typically been through 1-2 rounds of `reformulate_query_node` — which *deliberately* biases rewrites toward the knowledge base's own domain phrasing (climate/economics/health terms) to help *KB* retrieval. That same bias actively hurt a general web search, producing noisy, off-topic results (climate-policy PDFs) mixed in with the genuinely relevant weather answer. Fixed by using the original, unmangled `state["question"]` for the web search instead — re-verified with clean, fully relevant results.
3. **The most significant bug of this priority, and one that would have affected real production traffic:** the semantic cache was unconditionally caching *every* answer, including transient-failure fallback messages. This was caught live, by accident, during testing — a campus DNS outage made Tavily genuinely unreachable mid-session, and the resulting "couldn't find anything on the web either" failure message got cached and was then served, confidently, as the "answer" to a *different, unrelated question* asked afterward on a working connection, long after the DNS issue had already resolved. Fixed with a shared `_is_cacheable()` check, excluding all known transient-failure sentinel messages (LLM-providers-down, KB-and-web-both-failed) from ever being written to cache, applied at both places the code caches an answer.

**Verification:** confirmed via multiple real, live questions through the deployed app — an accurate real-time weather report for a city (correct temperature, correctly labeled, clean relevant sources, `scope_flagged: false`) and a general-knowledge question about a TV show (accurate summary from Wikipedia/IMDb/Rotten Tomatoes, correctly labeled, `scope_flagged: false`) — including one case that exercised the CRAG-false-positive safety net directly, proving all three fixes work together correctly, not just in isolation.

**A deliberate non-change, explicitly considered and declined:** making `out_of_scope` skip retrieval and CRAG entirely and route straight to the web search (to reduce latency on obviously off-topic questions) was proposed, but rejected — it would reintroduce the exact router-false-negative risk Priority 1's hardening fix had removed (a real in-KB question the router mis-tags as `out_of_scope` would then skip the knowledge base entirely, with no CRAG safety net to catch the router's mistake). The current behavior — retrieval and CRAG always run regardless of router category — was kept, accepting the extra latency on genuinely out-of-scope questions as the cost of that protection.

**Files changed:** `graph/web_search_node.py` (new), `llm/config.py` (added `tavily_api_key`), `graph/scope_guard_node.py` (widened prompt), `graph/build_graph.py` (rewired `out_of_scope` target, added the `route_after_generate` conditional edge), `graph/nodes.py` (`route_after_generate`, original-question fix), `api/main.py` (Pipeline Inspector labeling, `_is_cacheable()`), `ragagent-frontend/src/App.jsx` (icon + live result-count annotation for this stage).

**Result:** out-of-scope questions now get a real, useful, clearly-labeled answer instead of a dead-end refusal — closing the loop on the original Plan 1 Step 2 roadmap intent (an `out_of_scope` route was always meant to fall back to a live web search, not a hard refusal) years after Plan 1 itself shipped a fixed message as an interim placeholder.
