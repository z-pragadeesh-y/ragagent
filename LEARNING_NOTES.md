...
## Post-Plan-2 Closeout: Runtime Artifacts and Git Hygiene

### Why `feedback.sqlite` Being Committed Was a Real (if Small) Mistake, Not Nitpicking

This project already had the correct instinct for this category of file — `checkpoints.sqlite` was never committed, and `.chroma/` was excluded from the very first `.gitignore` back in Phase 0, both for the same underlying reason: runtime-generated, continuously-growing, environment-specific data doesn't belong in version control. `feedback.sqlite` is exactly the same category of file, created by Step 5's own logging code — and it slipped through simply because `.gitignore` wasn't updated at the same time the new file-producing code was written. This is a useful, generalizable habit to take forward: any time new code is added that writes a new file to disk as a side effect (a cache, a database, a generated artifact), the accompanying `.gitignore` update is part of that same unit of work, not an afterthought to catch later. It was caught here before it caused real harm (a large, ever-growing binary file bloating repo history over time), but catching it after multiple commits — rather than at the first commit that introduced it — is exactly the kind of small process gap worth remembering for next time.

### Why a Dedicated, Clearly-Labeled Commit Mattered

Untracking `feedback.sqlite` was done as its own isolated commit, separate from any other change, with a message explaining both what changed and why, by direct analogy to the existing `checkpoints.sqlite` precedent. This matters for the same reason atomic, well-labeled commits matter throughout real software engineering: a future reader of `git log` (including a future version of the person building this project) can immediately understand *why* this file disappeared from tracking without needing to reconstruct context from an unrelated, bundled commit message. Given how thoroughly this project's `README.md` and `LEARNING_NOTES.md` already document every phase's reasoning, a clean git history is the natural final layer of that same documentation discipline — visible directly in the version control tool itself, not just in prose written alongside it.

### The Project, Looked at as a Whole

Closing out Plan 2 is a reasonable point to step back and name what actually got built across the full arc — not as a victory lap, but because recognizing the shape of a finished body of work is itself a useful skill. This project moved through: a naive baseline (Phase 1) that was deliberately kept naive so its real limitations could be observed firsthand rather than assumed; a structural foundation (Phase 2's graph) built *before* it looked necessary, specifically so later branching and looping wouldn't require a rewrite; incremental intelligence layered on top (Phases 3-4, Plan 1) each addressing a specific, previously-documented gap rather than guessing at improvements; a real operational crisis (Groq quota exhaustion) turned into a genuine piece of infrastructure (multi-provider failover) rather than being patched around repeatedly; a rigorous, honest measurement layer (Plan 1 Step 4) that didn't loosen its own standards even when the numbers were inconvenient; and finally, a trust-and-observability layer (Plan 2) that made the system's behavior verifiable and visible rather than just functionally correct. The through-line across all of it, worth carrying into any future project: build the simple thing first, verify it for real rather than assuming it works, document what's broken as clearly as what's fixed, and let each documented limitation become the specification for the next piece of work rather than a problem to quietly patch around.

## Plan 3 — Production Deployment & Live Enhancements

### The Big Picture: A Working Pipeline and a Deployed Product Are Different Achievements

Everything through Plan 2 proved the *pipeline logic* was correct — verified via `python -m graph.test_cases` and direct `curl` calls. None of that proves a *deployed, publicly-reachable product* actually works, because an entire category of failure mode only exists once real infrastructure, real networks, and real users are involved: memory ceilings under concurrent load, CORS policies browsers enforce that `curl` never triggers, cold-start latency, DNS resolution failing on a specific network, a semantic cache persisting a transient failure forever. Plan 3's real lesson is that "it works" has to be re-earned at each new layer of reality the system touches — passing the golden-set eval didn't predict the Railway OOM crash, and passing a local `curl` test didn't predict the CORS rejection the first time a real browser hit the API.

### Concept: A Memory Ceiling Isn't a Bug to Patch, It's a Signal to Re-Architect

The Railway OOM crashes could have been chased with smaller and smaller workarounds (lazy-load the reranker, shrink the embedding batch size, and so on) — but the actual diagnosis (an embedding model + reranker + BM25 index + multi-provider LLM manager simply don't comfortably fit in 1GB) meant every one of those workarounds would have been treating a symptom of a hard resource ceiling, not the cause. Migrating to a platform with a larger memory allowance was the correct fix precisely because it addressed the actual constraint instead of shaving memory off the current design piece by piece.

### Concept: Every New Platform Has Its Own Undocumented Assumptions

Four distinct Cloud-Run-specific bugs (the `$PORT` env var expansion issue, the `.gcloudignore` vs `.gitignore` distinction, missing env vars carried over from a deleted platform, and Hugging Face's rate limit under a shared outbound IP) all shared the same shape: each one was invisible locally, and each one was a genuine, specific assumption the new platform made that the old one didn't share. The generalizable habit here isn't "know Cloud Run's quirks in advance" — it's "expect a new deployment platform to have its own undocumented assumptions, and treat the first deployment as a debugging pass, not a formality."

### Why the Accidental API Key Exposure Was Handled as a Real Incident, Not a Typo

Pasting real API keys into a chat session is exactly the kind of small, easy-to-dismiss mistake that's tempting to just note and move past once the immediate conversation is over. Treating it as a genuine security incident — rotating both keys at their source (not just "being more careful next time") and confirming the new values were live everywhere the old ones had been — is the correct default response regardless of how low the actual risk of misuse might have been in this specific case. The cost of over-reacting to a leaked credential is a few minutes; the cost of under-reacting is unbounded.

### Concept: A Router's Single-Shot Classification Needs an Evidence-Based Safety Net

Priority 1's most important fix — letting retrieval always run instead of trusting the router's category to gate it — is a specific instance of a general pattern worth naming: any single LLM call making a binary decision *before* real evidence is available (here, "is this in-scope?" decided before ever looking at what retrieval actually returns) will have a false-negative rate above zero. The fix wasn't "make the router smarter" (a losing battle against inherent LLM classification noise) — it was moving the actual gating decision to a point in the pipeline where real evidence (the retrieved-and-graded documents) exists to decide with, and treating the router's output as a *routing hint*, not a *final verdict*. The same shape shows up again in Priority 4's CRAG-false-positive safety net: grading occasionally passes bad chunks, so a second checkpoint downstream (checking for the model's own honest refusal phrase) catches what the first one missed, rather than trying to make the first checkpoint perfect.

### Concept: Streaming a Graph's Real Execution vs. Faking Progress

The original Pipeline Inspector's fixed-timing animation is a common, understandable shortcut — it "looks right" for the common case and is far simpler to build than real event streaming. The problem is exactly what showed up in testing: it silently lies on every branch it wasn't hardcoded to know about. The actual fix — LangGraph's `stream(stream_mode="updates")` — was already available and *synchronous*, meaning it didn't require the larger architectural change of migrating the checkpointer to an async variant. The generalizable lesson: before reaching for a fake-progress shortcut, check whether the underlying system already exposes real incremental state — it often does, and it's usually not much harder to wire up than the fake version once you know it's there.

### Why Reusing citation_node's Existing Metadata Shape for Web Results Mattered

`web_search_node.py` could have built a parallel citation/reference system specifically for web-sourced content, distinct from the knowledge-base one. Instead, Tavily results are wrapped as the exact same `Document` object shape (`source`, `document_title`, `section_title`, `domain_tag`) the citation node already reads for KB chunks — meaning `citation_node.py` itself needed zero changes to support an entirely new answer source. This is the same principle Priority 1's upload pipeline followed (reusing `structured_chunker.chunk_documents_structured()` unchanged rather than writing a parallel chunker) applied again: when a new capability can be shaped to fit an existing, already-correct code path instead of duplicating it, the new capability inherits all of that path's existing correctness (and existing bugs get fixed once, not twice).

### The Real Cost of an Unconditionally-Cached Failure

The semantic cache bug found during Priority 4 testing is worth sitting with, because it's a failure mode that's easy to miss in design review and only shows up under real, slightly unlucky conditions: a transient failure (a DNS outage, a provider being briefly down) isn't just wrong in the moment — if it's cached, it becomes a *permanent* wrong answer, silently served to every future similar question, with no natural expiry and no signal to the user that anything is stale. The fix (never cache a known transient-failure sentinel message) is simple once identified, but the bug itself was only caught because a real network outage happened to occur *during* testing — a reminder that some classes of bug genuinely require an unlucky real-world condition to surface, and can't be found by code review or golden-set testing alone.

### Why a Considered-and-Declined Change Is Still Worth Documenting

Priority 4 ended with an explicit request to skip retrieval/CRAG entirely for `out_of_scope` questions, to cut latency — and an explicit decision *not* to make that change, because it would reopen the exact false-negative risk Priority 1 had just closed. Recording the decision *not* to make a change, and why, is as valuable as recording a change that was made: without it, a future pass at "optimizing" this same code path would have no record that this exact trade-off was already considered and deliberately rejected, and might re-introduce the same regression the first time around.
